...
import type { LinkedProjectResourcePlan, Project, ProjectDraft, ProjectKind, ProjectPathCheck, Server } from '../types/models'
...
export function ProjectForm({ initial, projects, servers, onClose, onSave }: {
  ...
  const [resourceChecks, setResourceChecks] = useState<Record<string, Record<string, ProjectPathCheck>>>({})
  ...
  const availableModels = projects.filter((project) => project.kind === 'model' && project.id !== initial?.id)
  const availableResources = [...availableDatasets, ...availableModels]
  function setKind(kind: ProjectKind) { setDraft((current) => ({ ...current, kind, datasetIds: kind === 'project' ? current.datasetIds : [], modelIds: kind === 'project' ? current.modelIds : [] })) }
  ...
  function resourceProbeDraft(resource: Project): ProjectDraft {
    return {
      id: resource.id,
      name: resource.name,
      kind: resource.kind,
      ...
      targets: draft.targets
        .filter((target) => target.serverId !== resource.sourceServerId)
        .map((target) => ({
          serverId: target.serverId,
          path: resource.targets.find((item) => item.serverId === target.serverId)?.path
            ?? defaultProjectTargetPath(resource.sourcePath, resource.name),
        })),
    }
  }

  function missingResourceTargets(resource: Project) {
  ...
  async function detectLinkedResources(resourceIds = [...draft.datasetIds, ...draft.modelIds]) {
    ...
    const generation = ++resourceCheckGeneration.current
    setCheckingResources(true)
    try {
      const results = await Promise.all(resources.map(async (resource) => {
        const probe = resourceProbeDraft(resource)
        try {
          const checks = await api.probeProjectPaths(probe)
          return [resource.id, Object.fromEntries(checks.filter((check) => check.serverId !== resource.sourceServerId).map((check) => [check.serverId, check]))] as const
        } catch (reason) {
          return [resource.id, Object.fromEntries(probe.targets.map((target) => [target.serverId, {
            serverId: target.serverId,
      ...
      if (generation === resourceCheckGeneration.current) setResourceChecks((current) => ({ ...current, ...Object.fromEntries(results) }))
    } finally {
      if (generation === resourceCheckGeneration.current) setCheckingResources(false)
    }
  }

  useEffect(() => {
    if (draft.kind !== 'project' || (draft.datasetIds.length === 0 && draft.modelIds.length === 0) || draft.targets.length === 0) {
      resourceCheckGeneration.current += 1
  ...
  }, [draft.kind, draft.datasetIds.join('|'), draft.modelIds.join('|'), draft.targets.map((target) => `${target.serverId}:${target.path}`).join('|')])
  ...
  async function detectConfiguration() {
    await Promise.all([
      detectPaths(),
      draft.kind === 'project' && (draft.datasetIds.length > 0 || draft.modelIds.length > 0) ? detectLinkedResources() : Promise.resolve(),
    ])
  }
  ...
  async function submit(syncAfterSave: boolean, event?: FormEvent) {
    ...
    setError(null)
    const linkedResources = availableResources.filter((resource) => (resource.kind === 'dataset' ? draft.datasetIds : draft.modelIds).includes(resource.id)).map((resource) => {
      const targets = missingResourceTargets(resource)
      return {
  ...
  const missingResources = availableResources.filter((resource) => (resource.kind === 'dataset' ? draft.datasetIds : draft.modelIds).includes(resource.id) && missingResourceTargets(resource).length > 0)
  const plannedDatasetSyncCount = missingResources.filter((resource) => resource.kind === 'dataset' && resourceSyncIds.has(resource.id)).length
  const plannedModelSyncCount = missingResources.filter((resource) => resource.kind === 'model' && resourceSyncIds.has(resource.id)).length
  const plannedResourceSummary = [plannedDatasetSyncCount > 0 ? `${plannedDatasetSyncCount} 个数据集` : '', plannedModelSyncCount > 0 ? `${plannedModelSyncCount} 个模型` : ''].filter(Boolean).join('、')

  function renderResourceLinks(kind: 'dataset' | 'model') {
    const resources = kind === 'dataset' ? availableDatasets : availableModels
    const selectedIds = kind === 'dataset' ? draft.datasetIds : draft.modelIds
    const label = kind === 'dataset' ? '数据集' : '模型'
    const missing = missingResources.filter((resource) => resource.kind === kind)
    const allMissingPlanned = missing.length > 0 && missing.every((resource) => resourceSyncIds.has(resource.id))
    return <section className="project-dataset-links" aria-label={`关联${label}`}>
      <header><div><strong>关联{label}</strong><small>缺失副本可随“保存并同步”一并补齐</small></div><div className="project-dataset-links__actions">{checkingResources && <span>检查中…</span>}{missing.length > 1 && <button type="button" className="button button--secondary button--small" disabled={checkingResources} onClick={() => setResourceSyncIds((current) => { const next = new Set(current); missing.forEach((resource) => allMissingPlanned ? next.delete(resource.id) : next.add(resource.id)); return next })}>{allMissingPlanned ? '清除补齐' : '全部补齐'}</button>}</div></header>
      {resources.length > 0 ? <div className="project-dataset-list">{resources.map((resource) => {
        const linked = selectedIds.includes(resource.id)
        const checks = resourceChecks[resource.id] ?? {}
        ...
        return <div key={resource.id} className={`project-dataset-row${initial ? ' project-dataset-row--editing' : ''}${linked ? ' is-selected' : ''}`}>
          <label className="project-dataset-row__identity"><input type="checkbox" checked={linked} onChange={(event) => { const selected = event.target.checked; const field = kind === 'dataset' ? 'datasetIds' : 'modelIds'; setDraft((current) => { const currentIds = current[field]; return { ...current, [field]: selected ? [...currentIds, resource.id] : currentIds.filter((id) => id !== resource.id) } }); if (!selected) setResourceSyncIds((current) => { const next = new Set(current); next.delete(resource.id); return next }) }} />{kind === 'dataset' ? <Database size={13} /> : <Box size={13} />}<span><strong>{resource.name}</strong><small>{resource.sourcePath}</small></span></label>
          <span className={`project-dataset-row__status${problem ? ' is-warning' : checked && linked ? ' is-found' : ''}`}>{status}</span>
          {linked && checked && missingCount > 0 && <label className="project-dataset-row__sync"><input type="checkbox" checked={willSync} onChange={(event) => setResourceSyncIds((current) => { const next = new Set(current); if (event.target.checked) next.add(resource.id); else next.delete(resource.id); return next })} /><span>本次补齐</span></label>}
        </div>
      })}</div> : <p>暂无可关联的{label}。</p>}
    </section>
  }
  return (
      ...
      <section ref={sheetRef} className="sheet project-form-sheet" role="dialog" aria-modal="true" aria-labelledby="project-form-title">
        ...
        <form className="project-form" onSubmit={(event) => void submit(false, event)}>
          <div className="project-form__body">
            ...
            <div className="project-target-heading"><div><strong>目标服务器</strong><small>可同时选择多台，每台服务器可使用不同目录</small></div><div className="project-target-heading__actions"><span>已选 {draft.targets.length} / {eligibleTargets.length} 台</span><button type="button" className="button button--secondary button--small" disabled={draft.targets.length === eligibleTargets.length} onClick={selectAllTargets}>全选</button><button type="button" className="button button--secondary button--small" disabled={draft.targets.length === 0} onClick={() => { setDraft((current) => ({ ...current, targets: [] })); setChecks({}) }}>清空</button><button type="button" className="button button--secondary button--small" disabled={checking || checkingResources || draft.targets.length === 0} onClick={() => void detectConfiguration()}><FolderSearch size={14} />{checking || checkingResources ? '检查中…' : '检查配置'}</button></div></div>
