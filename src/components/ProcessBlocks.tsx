...
function ProcessHeader({ kind, count, terminatingPid, currentLabels }: { kind: 'gpu' | 'cpu'; count: number; terminatingPid?: number; currentLabels: boolean }) {
  return <header className="panel__header process-block__header"><div><span>{kind === 'gpu' ? <TerminalSquare /> : <Cpu />}</span><div><h3>{currentLabels ? '当前 ' : ''}{kind === 'gpu' ? 'GPU 进程' : 'CPU 进程'}</h3><p>{kind === 'gpu' ? `${count} 个计算进程 · 按任务颜色分组` : `${count} 个非系统进程 · 内存大于 1 GB`}</p></div></div>{terminatingPid !== undefined && <span className="process-block__terminating" role="status" aria-live="polite"><LoaderCircle className="spin" size={15} />正在结束 PID {terminatingPid}</span>}</header>
}
