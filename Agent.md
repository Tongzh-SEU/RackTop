...
`1.24.4` 版本中与 Windows 窗口壳层、无边框窗口、标题栏、窗口控制、Windows 启动尺寸、DPI 适配、控制台隐藏和安装器兼容性相关的改进，默认仅针对 Windows 实现和验证。除非用户明确说明“通用”或明确要求 macOS 同步，否则不得修改 macOS 的窗口行为、Overlay 标题栏、红绿灯位置、初始尺寸或布局。版本说明中也必须标明该类修复为 Windows 专属。
